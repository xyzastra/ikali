...
const Community = () => {
  ...
  const {
    data: projects,
    isLoading
  } = useUserProjects(filter === "collaborative" ? {
    type: "collaborative"
  } : filter === "personal" ? {
    type: "personal"
  } : filter === "featured" ? {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <SectionHeader title="Community Projects" description="Discover, share, and collaborate on projects" />
          {user && <Link to="/community/submit">
        ...
        {isLoading ? <div className="text-center py-12 text-muted-foreground">Loading projects...</div> : !projects?.length ? <div className="text-center py-12">
            ...
            {/* Featured Section */}
            {filter === "all" && featuredProjects.length > 0 && <div className="mb-12">
                <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                  <Star className="h-4 w-4 text-yellow-500" /> Featured
                </h3>
                <div className="grid gap-4 md:grid-cols-2">
                  {featuredProjects.slice(0, 2).map(project => <ProjectCard key={project.id} project={project} featured />)}
                </div>
              </div>}

            {/* All Projects Grid */}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {regularProjects.map(project => <ProjectCard key={project.id} project={project} />)}
            </div>
          </>}

        {!user && <div className="mt-8 p-6 border border-border rounded-lg bg-muted/30 text-center">
...
const ProjectCard = ({
  ...
  return <Link to={`/community/${project.id}`} className={`group block border border-border rounded-lg overflow-hidden bg-card hover:border-primary/50 transition-all hover:shadow-md ${featured ? "md:flex" : ""}`}>
      ...
      <div className={`p-4 ${featured ? "md:flex-1" : ""}`}>
        ...
        {project.description && <p className="text-sm text-muted-foreground line-clamp-2 mb-3">
            {project.description}
          </p>}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {project.looking_for_collaborators && <Badge variant="outline" className="text-xs text-green-600 border-green-600">
              Looking for help
            </Badge>}
          {project.is_featured && <Badge variant="outline" className="text-xs text-yellow-600 border-yellow-600">
