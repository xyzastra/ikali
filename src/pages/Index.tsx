...
const Index = () => {
  ...
  return <div className="min-h-screen">
      ...
      <div className="bg-background">
        ...
        <section className="container mx-auto max-w-6xl border-b border-border py-[20px] px-[18px]">
          <h2 className="text-3xl font-serif font-bold mb-8">Featured</h2>
          <Carousel>
            {featuredProjects.map(item => <CarouselItem key={`${item.type}-${item.id}`}>
                <FeedItem title={item.title} description={item.description} date={item.date} path={`/${item.type === 'project' ? 'projects' : 'journal'}/${item.id}`} tags={item.tags} coverImage={item.coverImage} readingTime={item.readingTime} />
              </CarouselItem>)}
          </Carousel>
        </section>
        ...
        <section className="container mx-auto px-4 py-8 sm:py-12 max-w-4xl">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
            {sections.map(section => <Link key={section.path} to={section.path} className="group border border-border rounded-lg p-4 shadow-sm hover:shadow-md hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 active:scale-[0.97] bg-primary">
                <h3 className="text-sm sm:text-base font-semibold mb-1 transition-colors text-accent">
                  {section.title}
                </h3>
